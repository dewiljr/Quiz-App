...
function App() {
  ...
  useEffect(() => {
    if (showScore) {
      ...
      if (score >= 7) {
        resultMessage = "Sizi Restoranımda İşe Alıyorum.";
      } else if (score >= 3) {
        resultMessage = "Aman Ağzımızın Tadı Bozulmasın Ali Rıza Bey";
      } else {
        resultMessage = "Siz Benim Quizimden Geçebileceğinizi Mi Sandınız?";
      }
    ...
    <div className="app">
      {!quizStarted ? (
        <div className="start-screen">
          <h1>Sen Benim Quizimi Çözebileceğini Mi Sandın?</h1>
          <button id='start' onClick={handleStartQuiz}>Ama Neden?</button>
        </div>
      ) : showScore ? (
        <div className="score-section">
          <h2>Bizde Cevaplar Açık Büfe Gibidir.</h2>
        ...
        <div className="answers-section">
          ...
          <button onClick={handleRestartQuiz}>Olur da Yeniden Başlatmak İstersen</button>  
        </div>
      ) : (
        <div>
